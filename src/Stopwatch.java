...
public class Stopwatch implements Runnable{
	...
	//Timer object is used to schedule tasks. In this case, the task will check the time every second and update the stopwatch accordingly
	private Timer timer;
	...
	//startTimer captures the current time, which is the Start Time, then passes it to the task object before scheduling the task with the timer.
	...
	/*stopTimer stops the timer without stopping the thread. I planned for this to be used during the game over screens to display the final time
	 * But implementation still needs to be worked out for that.
	 */
	...
	//updateTime is called by the inner TimerTask class. It uses the time calculated by the TimerTask to update the timer label.
	public void updateTime(LocalTime time) {
		currTime = time;
		label.setText("Current Time: " + dtf.format(currTime));
	}
	
	//display is called by the game class to retrieve the timer panel for use in the game window
	public JLabel display() {
		return label;
	}
	
	/* run() is a function inherited from the Runnable interface.
	 * It is called automatically by the Thread.start() method.
	 */
	public void run() {
		/*Test
	...
	/* stopThread is called by the Game class to stop the timer thread. This should also be called by
	 * the exit button on the gui to stop the thread before shutting the program down. This is because threads are unaffected
	 * when other threads close down or stop. If this is not called properly, the GUI will close but the thread will remain.
	 * This can be done using DO_NOTHING_ON_CLOSE and a WindowListener.
	 */
	...
	/*
	 * UpdateTimer is an inner class of Stopwatch. This is because timers require separate TimerTask objects in order to work. 
	 * In this case, we need the time to be passed from the TimerTask back to the stopwatch, so making it an inner class works best.
	 */
	class UpdateTimer extends TimerTask{
		...
		/*
		 * checkValues is called in order to make sure all of the subtracting works properly. It basically performs the same
		 * function that a carry would in subtraction.
		 */
		...
		/*
		 * run() is called by the timer object in the outer class when the scheduled time has come.
		 */
		@Override
		public void run() {
			...
			String timeString = decFormat.format(hours) + ":" + decFormat.format(minutes) + ":" + decFormat.format(seconds);
